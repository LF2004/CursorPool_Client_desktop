...
function beginTurn(config = {}, requestId = '', workspaceRoot = '', capture = null) {
  ...
  if (capture?.mode) {
    const normalizedMode = String(capture.mode || '').trim();
    if (normalizedMode === 'AGENT_MODE_PLAN') conversation.state.mode = 'plan';
    else if (normalizedMode === 'AGENT_MODE_ASK') conversation.state.mode = 'ask';
    else if (normalizedMode === 'AGENT_MODE_AGENT') conversation.state.mode = 'agent';
  }
