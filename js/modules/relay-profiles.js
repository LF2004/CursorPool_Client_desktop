...
export const RELAY_FIELD_HINTS = {
  ...
  maxCompletionTokens: 'OpenAI 模型最大输出 Token 数。留空时使用 API 默认值。',
  anthropicMaxTokens: 'Anthropic 模型最大输出 Token 数。留空时使用 API 默认值。',
  anthropicThinkingEffort: 'Anthropic 思考强度（低/中/高/极高/Max）。控制 extended thinking 的计算量。',
  openAIExtraParamsEnabled: '是否启用 OpenAI 额外参数 JSON（将 JSON 内容合并到请求体）。',
  openAIExtraParamsJSON: 'OpenAI 额外参数 JSON，将合并到请求体的顶层（如 {"service_tier": "auto"}）。',
  anthropicExtraParamsEnabled: '是否启用 Anthropic 额外参数 JSON（将 JSON 内容合并到请求体）。',
  anthropicExtraParamsJSON: 'Anthropic 额外参数 JSON，将合并到请求体的顶层。',
  customHeadersEnabled: '是否启用自定义请求头（将 JSON 中的键值对作为 HTTP 请求头发送）。',
  customHeadersJSON: '自定义请求头 JSON，格式为 {"Header-Name": "value"}。',
...
function normalizeProfile(raw) {
  ...
  return {
    ...
    maxCompletionTokens: Math.max(0, Number(raw.maxCompletionTokens) || 0),
    anthropicMaxTokens: Math.max(0, Number(raw.anthropicMaxTokens) || 0),
    anthropicThinkingEffort: ['low', 'medium', 'high', 'xhigh', 'max'].includes(raw.anthropicThinkingEffort) ? raw.anthropicThinkingEffort : 'medium',
    openAIExtraParamsEnabled: Boolean(raw.openAIExtraParamsEnabled),
    openAIExtraParamsJSON: String(raw.openAIExtraParamsJSON || '{}').trim() || '{}',
    anthropicExtraParamsEnabled: Boolean(raw.anthropicExtraParamsEnabled),
    anthropicExtraParamsJSON: String(raw.anthropicExtraParamsJSON || '{}').trim() || '{}',
    customHeadersEnabled: Boolean(raw.customHeadersEnabled),
    customHeadersJSON: String(raw.customHeadersJSON || '{}').trim() || '{}',
    ...
    const payload = {
      ...
      configs: (store.configs || []).map((item) => {
        ...
        return {
          ...
          anthropicThinkingEffort: p.anthropicThinkingEffort,
          openAIExtraParamsEnabled: p.openAIExtraParamsEnabled,
          openAIExtraParamsJSON: p.openAIExtraParamsJSON,
          anthropicExtraParamsEnabled: p.anthropicExtraParamsEnabled,
          anthropicExtraParamsJSON: p.anthropicExtraParamsJSON,
...
function syncLegacyActiveConfig(store) {
  ...
  try {
    localStorage.setItem(LEGACY_KEY, JSON.stringify({
      ...
      maxCompletionTokens: active.maxCompletionTokens,
      anthropicMaxTokens: active.anthropicMaxTokens,
      anthropicThinkingEffort: active.anthropicThinkingEffort,
      openAIExtraParamsEnabled: active.openAIExtraParamsEnabled,
      openAIExtraParamsJSON: active.openAIExtraParamsJSON,
      anthropicExtraParamsEnabled: active.anthropicExtraParamsEnabled,
      anthropicExtraParamsJSON: active.anthropicExtraParamsJSON,
      customHeadersEnabled: active.customHeadersEnabled,
      customHeadersJSON: active.customHeadersJSON,
      notes: active.notes,
    }));
  } catch {
    /* ignore */
